 ...
 */
class DashboardAdapter(private val onSelect: (DashboardItemModel) -> Unit) : RecyclerView.Adapter<DashboardAdapter.DashboardHolder>() {

    private var items = listOf<DashboardItemModel>()

    inner class DashboardHolder(private val binding: DashboardGridItemBinding, onSelect: (Int) -> Unit) : RecyclerView.ViewHolder(binding.root) {
        init {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): DashboardHolder {
        val binding: DashboardGridItemBinding = DataBindingUtil.inflate(LayoutInflater.from(parent.context), R.layout.dashboard_grid_item, parent, false)
        return DashboardHolder(binding) {
            onSelect(items[it])
        }
    }

    override fun onBindViewHolder(holder: DashboardHolder, position: Int) = holder.bind(items[position])
